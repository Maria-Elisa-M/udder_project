...
# files in the lab computer had a different size we will not use these for training
files_df = files_df[files_df.computer != "lab"]
print(f"There are {len(files_df)} videos after removing lab pc")
...
cow_list = np.concatenate((np.unique(files_df[files_df.computer == "guilherme"].cow)[:25], \
